...
CEDAR alone scores baseline ROC-AUC=0.89, trained ROC-AUC=0.92). See
...
def _load_test_rows() -> tuple[list[PILImage], list[str], list[bool]]:
    ...
    for row in cedar["test"]:
        ...
        is_genuine.append(bool(row["is_genuine"]))
    return images, writer_ids, is_genuine
...
def _build_random_forgery_pairs(
    ...
    for writer in writers:
        ...
        for impostor_writer in chosen_writers:
            ...
            pairs.append((reference, impostor_index, 0))
    return pairs
...
def main(cfg: DictConfig) -> None:
    ...
    images, writer_ids, is_genuine = _load_test_rows()
    skilled_pairs = _build_skilled_forgery_pairs(writer_ids, is_genuine)
    random_pairs = _build_random_forgery_pairs(writer_ids, is_genuine)
