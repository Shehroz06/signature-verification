"""Trains the embedding model on this project's registered corpus
(signature_forgery) -- corpus selection is a Hydra override, kept even with
one entry so a second corpus can be added later without a new script:
...
and can be overridden the same way, e.g.:
    ...
    uv run python -m scripts.train corpus=signature_forgery training.num_epochs=30
    uv run python -m scripts.train corpus=signature_forgery training.device=cuda \\
        training.dataloader.num_workers=4 training.dataloader.pin_memory=true
